...
module.exports = {
	groupRecs : function(origRecs,exposedRecs) {
		exposedRecs.splice(0);
		var groups = [ [ origRecs[0] ] ], prevGrpRec = origRecs[0], currGrp = 0;
		for ( var i = 1; i < origRecs.length; i++) {
			var rec = origRecs[i];
			if (rec.refAirlineCode !== prevGrpRec.refAirlineCode
					|| prevGrpRec.totPricePerAdt !== rec.totPricePerAdt) {
				groups.push([ rec ]);
				prevGrpRec = rec;
				currGrp++;
			} else {
	...
	populateIndices : function(rec, recIndex) {
		var i = 0;
		rec.index = recIndex;
